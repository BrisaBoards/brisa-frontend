...
var BrisaMessager = function(cache, views) {
  ...
  this.Register = function(view, add, update, destroy) {
    ...
    for (var i in view.updates) {
      this.SendUpdate(reg, view.updates[i]);
    }
  };
  this.Event = function(onEvent, handler) {
    // gen unique id
    // add to events dict
  };
  ...
  this.SendUpdate = function(active, msg) {
    ...
    if (msg.m == 'cmt') {
      if (idx != -1) {
        var ent = view.entries[idx];
        var cnt = 0;
        if (msg.a == 'create') cnt = 1;
        if (msg.a == 'destroy') cnt = -1;
        ent.data.comment_count += cnt;
      }
      if (active['cmt'] && idx) active['cmt'](msg);
    } else if (view.parent && view.parent.data.id == msg.id) {
